...
def load_documents(data_dir: str) -> List[Document]:
  """Load documetns from a documentary of markdown files
  ...
  documents = [
    UnstructuredMarkdownLoader(file_path=file_path).load()[0] for file_path in md_files
  ]
  return documents
...
def chunk_documents(
  ...
  """Spit documents into chunks

  Args:
    documents(List[Document]): A list of documents to split into chunks
    chunk_size(int, optional): The size of each chunk Defaults to 500.
    chunk_overlap(int, optional): The number of tokens to overlap between chunks. Default to 0
...
def create_vector_store(
  ...
  Args:
    ...
    vector_store_path(str, optional): The path to the vector store. Default to "./vector_store".
